                            ...
                            for s in range(0, window_size):
                                for c in range(0, num_input_channel):
                                    ...
                                    if partial_sum[n][m][p][q] > 524287: 
                                        ...
                                        print("OVERFLOW +")
                                    elif partial_sum[n][m][p][q] < -524288:
                                        print(partial_sum[n][m][p][q])
...
if __name__ == "__main__":
    ...
    # plot all the output activation
    activations_to_plot = np.empty(0)
    for i in range(7):
        activations_to_plot = np.append(activations_to_plot, network.output_collection[i].flatten())
    print("partial sum max: ", np.amax(activations_to_plot), "partial sum min: ", np.amin(activations_to_plot))
    plt.hist(activations_to_plot)
    plt.yscale('log')
    plt.savefig('a.png')
